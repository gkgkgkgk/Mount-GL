...
void Universe::AssignOccluders()
{
	for (int i = 0; i < bodies.size(); i++)
	{
		...
		{
			if (i == j || j == emissiveBodyIndex)
				continue; // A body should never been its own occluder, nor should the light source been an occluder

			bodies.at(i)->occluders.push_back(bodies.at(j));
			if (bodies.at(i)->occluders.size() >= MAX_OCCLUDERS)
				break;
		}
	}
}
...
{
	if (timeScale > 0 && deltaTime < 0.05F)
		updateBodies(deltaTime * timeScale); // If deltaTime is too high, don't update the bodies as the large deltaTime will distort orbits
}
...
void Universe::updateBodies(double deltaTime)
{
	...
	for (unsigned int i = 0; i < bodies.size(); i++)
	{
		...
		if (body->affectedByGravity)
		{
			...
			{
				if (i == j)
					continue; // No need to calculate the gravitational pull of a body on itself as it will always be 0.
				MassBody *otherBody = bodies.at(j);

				if (!otherBody->affectsOthers)
